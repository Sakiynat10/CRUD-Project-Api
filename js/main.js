...
function getTeacherRow({firstName , groups ,phoneNumber , email ,telegram , twitter, lastNmae ,github, avatar, field , isMarried , id}){
    return `
...
async function getTeachers(){
    ...
    try{
        ...
        const p2 = await axios.get( `${ENDPOINT}teacher`, { params: { search, isMarried } } );
        ...
        editTeacherBtns.forEach((editBtn) => {
            editBtn.addEventListener('click' , async function(){
                ...
                let {data:teacher} =  await axios.get(`${ENDPOINT}teacher/${selected}`)
